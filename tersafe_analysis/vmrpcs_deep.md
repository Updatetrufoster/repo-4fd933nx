...
## 11.6 VMRPCS / MRPCS 关联地址全表（call-graph 逆向）

以下由 17 个已确认 MRPCS/VMRPCS 根函数 BFS 展开其 `bl` 目标、逐一分类得到。**标 ★ = 完整性/校验（checksum/CRC/hash）相关。**

### A. 顶层接收 / 分发 / 命令
| 地址 | 作用 |
|---|---|
| `0x42A700` | MRPCS 处理线程 / 下载分发入口 |
| `0x42A5D8` | 顶层接收器（CRC 去重→ZIP→VMRPCS→命令分派→复位） |
| `0x42AD54` | 包解析/分发（ZIP 判定 + switch common/single/ack） |
| `0x429AA4` | 前置门控（决定是否走 `0x42AD54`） |
| `0x4297D8` | 解析前置校验 |
| `0x429714` | ★「该 CRC 已是当前生效下发」判定（早退） |
| `0x429D4C / 0x429E94 / 0x429FDC` | 空包→复位 common/single/其它规则容器 |
| `0x42A174` | 复位后收尾/通知 |

### B. 命令处理器 + 规则装载
| 地址 | 作用 |
|---|---|
| `0x42B570` | cmd=1 common 规则集处理 |
| `0x42B87C` | cmd=3 ack/心跳处理 |
| `0x42BAD0` | cmd=4 single 规则集处理 |
| `0x42BE5C` | 规则装载（common `ctx+0xc0` / single `ctx+0x108`） |
| `0x42C0B0 / 0x42C45C / 0x42CA8C / 0x42CC7C / 0x42CCB0 / 0x42CCE4 / 0x42CEBC / 0x42CED4 / 0x42CEFC / 0x42D010 / 0x42D258` | 规则容器读写/迭代公共子例程（3 个 handler 共用） |
| `0x42D28C / 0x39F710 / 0x376A84 / 0x376D88 / 0x375DBC / 0x375DE8` | 规则条目解析/存储 |
| `0x4486F0 / 0x4487EC` | handler 公共尾处理 |

### C. 解压 (zlib)
| 地址 | 作用 |
|---|---|
| `0x42A338` | 解压 `mrpcs.data`（隐藏串 `unzipmrpcs.data`） |
| `0x3D555C` | inflateInit |
| `0x3D5598` | inflate |
| `0x3D5738` | inflate（分支/reset） |
| `0x3D5778` | inflateEnd |

### D. 反混淆 + 校验（VMRPCS 验签核心）
| 地址 | 作用 |
|---|---|
| `0x3A2D18 / 0x3A2D28` | 解析容器 ctor / dtor |
| `0x3A2D38` | ★ 反混淆 `(⊕k1)+k2` + 头部校验和比对，返回解析对象 |
| `0x3A2FCC` | ★ `0x3A2D38` 内部校验子例程 |
| `0x2E5F08` | ★ **crc32lei 状态初始化**（全局状态 `0x563818`，锁 `0x5637F0`） |
| `0x2E6188` | ★ **crc32lei 更新**（`lsr#8`+`eor`，状态 `0x56381C`） |
| `0x2E60EC` | ★ **crc32lei 收尾**（位反射/finalize） |
| `0x2E6188`→表 | ★ 复用标准 CRC32 表 `0xe2158`（poly `0xEDB88320`） |
| `0x2E63D0` | 锁访问器（全局对象 `0x563C28`，供 handler 加锁用） |

### E. 去重缓存（CRC 作键）
| 地址 | 作用 |
|---|---|
| `0x3F5280` | ★ 去重缓存单例（`0x5663D0`） |
| `0x3F63F4` | ★ 去重「是否重复/已生效」判定 |
| `0x3F6104` | ★ 登记已处理包 |
| `0x3F620C` | ★ 计算去重摘要键（多步 hash 原语） |
| `0x3F67B0 / 0x3F68DC / 0x3F7AFC / 0x3F7BA8 / 0x3F7C00 / 0x3F7C8C / 0x3F7CB4` | ★ 摘要 update/final 原语 |
| `0x3FA004 / 0x3FA120` | ★ 去重缓存插入 |
| `0x3F6348` | 去重特性门控（配置 `mp_lf_nwthrd`，getter `0x392BB8`） |
| `0x428BD8` | MRPCS 主接收器单例（`0x566810`，锁 `0x566818`） |
| `0x42989C` | ★ 与当前生效集比对（遍历 `obj+0x840` 链表比 CRC/键） |

### F. VMRPCS 专包（异步任务）
| 地址 | 作用 |
|---|---|
| `0x42A0E8` | VMRPCS 魔数命中→投递异步任务 |
| `0x2EB80C` | 异步任务对象 ctor（state=7，分配 0x128，跳转表 `0x53AA50`） |
| `0x433A5C / 0x433CB8` | 任务/报告对象工厂 |

### G. 内存扫描 + 上报
| 地址 | 作用 |
|---|---|
| `0x3E06D0` | `/proc/self/maps` 扫描器（`r-xp`/`r--p`） |
| `0x3DEA94` | 扫描项上下文获取 |
| `0x434F3C` | 命中结果派发 |
| `0x434C44 / 0x434DB0` | 上报组包子例程 |
| `0x37CD70 / 0x37DA04` | 事件/通知（`sub_37DA04(_,5)` 唤醒扫描线程） |
| `0x37DACC / 0x37DADC / 0x37E0E4` | 通知/回调派发 |

### H. 流读取器 / 容器 / 内存原语（通用支撑）
| 地址 | 作用 |
|---|---|
| `0x3F878C / 0x3F88BC / 0x3F88E8 / 0x3F8AF8 / 0x3F8DB0 / 0x3F8DC8` | 字节流读取器（建/读 dword/读长度/关闭） |
| `0x4191C0 / 0x419368 / 0x419518 / 0x41943C / 0x41A0A4` | 容器工厂 / ctor / init（虚调 `vtable+0x10`） |
| `0x3864F4 / 0x386528 / 0x38655C / 0x386598 / 0x3865B0 / 0x3861A8 / 0x364088` | 迭代器 / 容器 / 引用计数 |
| `0x364BDC / 0x364C14` | mutex lock/unlock（`unk_566818`） |
| `0x364C84 / 0x364CEC` | `lock_guard` RAII ctor/dtor |
| `0x42B51C / unk_564988` | 解析器一次性 init |
| `0x508150 / 0x508200 / 0x508208 / 0x5081C0 / 0x5081C8` | operator new/delete |
| `0x2E06E4 / 0x2E497C` | 局部对象析构/临时缓冲 |

### I. 全局对象 / .bss 映射
| 地址 | 作用 |
|---|---|
| `0x5637F0` | ★ crc32lei 状态锁 |
| `0x563818 / 0x56381C` | ★ crc32lei 全局状态 |
| `0x563C28` | handler 加锁用全局对象 |
| `0x5663D0` | 去重缓存单例指针 |
| `0x566810` | MRPCS 主接收器单例指针 |
| `0x566818` (`unk_566818`) | 保护 `0x566810` 的互斥锁 |
| `0x566840` 附近 | 当前生效集链表头（`0x42989C` 遍历） |
| `unk_564988` | 解析器一次性 init 对象 |
| `0xe2158` | ★ 标准 CRC32 表（poly `0xEDB88320`），crc32lei/去重共用 |
| `0x53AA50` | VMRPCS 异步任务跳转表 |

### 校验/完整性小结（★项）
MRPCS 链上有 **两条独立 CRC/校验路径**：
1. **crc32lei（`0x2E5F08/0x2E6188/0x2E60EC`）**：对**整包原始字节**算 CRC，既做完整性校验又作**去重键**（`0x3F63F4` 查 / `0x3F6104`+`0x3FA004` 记 / `0x429714` 判当前生效）。失败即 `mrpcs_data_crc_error` 丢弃。
2. **VMRPCS 内部校验（`0x3A2D38/0x3A2FCC`）**：解压后对 payload 做 `(⊕k1)+k2` 反混淆并比对头部**校验和**，配合 `"VMRPCS"` 魔数 + 长度加扰验签，失败 `mrpcs_common/single_data_not_match`。

两者都复用标准 CRC32 表 `0xe2158`；证书/文件另走 MD5/SHA-256（见 `anti_tamper_crc.md`）。
