...
## 2.5 哈希原语深挖：CRC32 · MD5 · SHA-256（常量 / 函数 / 用途）

完整性校验用到 **3 套哈希原语**，全部按常量特征在二进制里定位到，并确认了各自函数入口与调用点。

### 2.5.1 常量表（.rodata 内确认）
| 常量 | 地址 | 算法 | 说明 |
|---|---|---|---|
| poly `0xEDB88320` 反射表 | `0xe2158` | **CRC32** | 标准反射 CRC32 表（256 项） |
| `0x67452301 / 0xefcdab89 / 0x98badcfe / 0x10325476` | `0x99d20` | **MD5** | MD5 初始状态 A/B/C/D（无第 5 字，故非 SHA-1） |
| `0x6a09e667 / 0xbb67ae85 / 0x3c6ef372 / 0xa54ff53a` | `0x9a1a0` | **SHA-256** | 初始 H0..H3 |
| `0x510e527f / 0x9b05688c / 0x1f83d9ab / 0x5be0cd19` | `0x9a050` | **SHA-256** | 初始 H4..H7 |
| `0x428a2f98 / 0x71374491 …` | `0xe255c` | **SHA-256** | 64 个轮常量 K（256 字节） |

> 无 MD5 K 表常量 → MD5 轮常量运行时计算（`floor(2^32*abs(sin))`），属常规实现。

### 2.5.2 CRC32 函数族（代码段校验主用）
| 函数 | 地址 | 调用数 | 用途 |
|---|---|---|---|
| `crc32_table()` | `0x489204` | — | 返回表 `0xe2158` |
| `crc32(buf,len)` | `0x489210` | **147** | 一次性 CRC，全库通用 |
| `crc32_cont(buf,len,seed)` | `0x48924c` | 6 | **续算**——`txt_seg_crc` 用它（调用点 `0x28e158`，在 `sub_28DE4C` 内） |
| `crc32_file(path,*out,limit,k)` | `0x489288` | 2 (`0x2bf4c8`,`0x4c4c48`) | 文件 4KB 分块 CRC——`apk_hash_*` 用它 |

### 2.5.3 MD5 原语
- 入口/变换：`0x4879f8`（引用 IV `0x99d20`）/ `0x4886ec`（一次性 MD5，**10 个调用点**）/ final `0x4897a4`。
- 用途：
  - **`cert_md5`**：对 APK 签名证书算 MD5（`cal_cert_md5`）→ 与内置 `official_cert_md5` 比对（§2.3）。
  - **ID/令牌生成**：如 `md5("%ld_%p")`（`0x4c68b8`）、`md5("%ld%ld%ld%ld")`（`0x4cf2b0`）生成会话/上报唯一标识。

### 2.5.4 SHA-256 原语
- init `0x489da8`（装入两半 IV `0x9a050`+`0x9a1a0`）；transform `0x48994c`（用 K 表 `0xe255c`，64 轮）；块处理封装 `0x489be8 / 0x489cb4 / 0x489d20`。
- **只有一个使用点：`0x2c0bb8`**（`bl 0x489da8 @0x2c0c5c`）——收集若干缓冲(经 `0x1e7dc4/0x1e7df0` 取)后算 32 字节摘要（`cmp #0x1f` 遍历 31→32 字节），形态为**上报/attestation 的签名摘要**（对应 `TssSDKOnRecvSignature` 挑战应答链）。相比 CRC/MD5，SHA-256 在本库用得最省，专用于强完整性签名。

### 2.5.5 三者在「代码段/完整性上报」中的分工
上报组包点 `0x4cbd40`（`sub_4cbd7c`）逐字段拼：
```c
// 模板: cert_md5=%s|apk_hash_1=0x%08x|apk_hash_2=0x%08x|txt_seg_crc=0x%08x
ldr w6, [x20, #0x84]     // @0x4cbde0 = txt_seg_crc = ctx+0x84（CRC32_cont 结果）
```
| 上报字段 | 算法 | 来源 |
|---|---|---|
| `txt_seg_crc` | **CRC32**(cont) | 内存 `.text` 逐页续算，`sub_28DE4C`→`ctx+0x84` |
| `apk_hash_1` / `apk_hash_2` | **CRC32**(file) | APK 本体 / inner_apk / so 文件 `crc32_file` |
| `cert_md5` | **MD5** | 签名证书 |
| `cert_md5_crc` | **CRC32(MD5)** | 对 cert_md5 再算 CRC |
| 上报签名摘要 | **SHA-256** | `0x2c0bb8` 收集数据 → 32B 摘要 |

一句话：**代码段/文件完整性用 CRC32（快、当指纹），证书用 MD5，整体上报用 SHA-256 做签名摘要**——CRC32 表 `0xe2158`、MD5 IV `0x99d20`、SHA-256 IV `0x9a050/0x9a1a0` + K `0xe255c` 均已逐字节定位。
